...
```
碎片留不留得下来，在 free() 那一刻就定了：
__free_one_page() 立刻算 buddy_pfn = pfn ^ (1 << order)，
伙伴也空闲就合并、进位、继续试 —— 根本轮不到规整登场。
...
**★ 2026-08-11 实测修正（原文这里写着"顺序释放会一路合并回大块、白干一场"，
实测不成立，已按 `holes.c` 头注释更正）**：

同样 512 MB / 64 KB 块 / 释放一半（Normal zone）：

| | order-0 | order-1 | order-3 | order-4 |
|---|---|---|---|---|
| 基线 | 406 | 234 | 8119 | 6350 |
| 顺序释放后 | 5000 | 5883 | 3254 | 2692 |
| 随机释放后 | **6360** | **6598** | **2516** | **2502** |

随机更狠，但**顺序也造出了大量碎片**。根因是**虚拟连续 ≠ 物理连续**：
顺序释放的是虚拟地址上连着的那一半，而背后的物理页是缺页时东一块西一块摘的，
合并只看 PFN 邻居，所以也合不起来。随机之所以仍更狠，是因为同一进程连续缺页时
内核倾向于在同一 pageblock 内摘相邻页，虚拟连续那一半物理上**仍有部分聚集**，
随机把这点残余规律也打散。（完整推导见 9.8 第 2 题）
...
**【一句话】** 因为碎片是在 **`free()` 当场**决定的：`__free_one_page()` 会立刻
去找伙伴合并。随机释放能把"物理上还残存的聚集"也打散，合并链断得更彻底。
**但注意 —— 顺序释放并不是白干（实测），见下。**
...
碎片能不能留下来，在这一刻就定了 —— **根本轮不到规整登场**。

**【展开·★ 2026-08-11 实测修正，这一条必须记准】**
原来的说法是"顺序释放会被一路合并回大块、白干一场"。**实测不成立。**
同样 512 MB / 64 KB 块 / 释放一半（Normal zone）：
...
随机确实更狠（低阶碎屑更多、中阶残留更少），但**顺序释放也造出了大量碎片**。

**原因是"白干"那个说法混淆了两件事：虚拟连续 ≠ 物理连续。**
"顺序"释放的是**虚拟地址**上连着的那一半，而这片虚拟内存背后的物理页
是缺页时从伙伴系统东一块西一块摘来的 —— **虚拟地址挨着，物理页并不挨着**。
`__free_one_page()` 的合并只看 **PFN 的邻居**，所以"顺序还"还回去的
其实是物理上散落各处的页，合并自然起不来。

**那随机为什么仍然更狠？** 因为同一进程连续缺页时，内核倾向于从同一个
pageblock 里摘相邻的页（migratetype 分组的副作用），
所以虚拟连续的那一半在物理上**仍有部分聚集**；顺序释放能捞回一些合并机会，
随机释放把这点残余规律也打散。

**【常见错答（第一遍就错在这）】** 把失败归因给"规整把它们合回去了"。
**不对 —— 根本轮不到规整，在 `free()` 里就已经合并完了。**
这是**分配器行为**，不是**规整行为**，发生在完全不同的时刻。

另外两条要记住：
1. **伙伴 ≠ 相邻**。PFN 1 和 2 相邻，但 `1^1=0`、`2^1=3`，它们不是伙伴，
   永远合不到一起。所以随机释放只是**概率上**破坏合并 ——
   `holes.c` 因此只释放一部分而不是全部。
2. **不要把"顺序释放白干"当结论说出去** —— 自己项目的实测数据就否掉了它，
   面试官真追问一句"那你测过吗"会很难看。正确的说法是
   "**随机更彻底，但差距没有想象中大，因为虚拟连续不等于物理连续**"。
