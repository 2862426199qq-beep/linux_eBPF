 ...
 *
 * 第 3 步的"随机"是这个程序的核心手法：
 *   - 随机释放：还回去的页的"伙伴"（buddy）大概率还被占着，
 *     __free_one_page() 的合并在第一步就断了，
 *     于是空闲页只能以小块的形式挂在低 order 的链表上。
 *
 * ★ 2026-08-11 实测修正（原注释把这里写过头了，必须纠正）
 *
 * 原注释说"顺序释放会被一路合并回大块、白干一场"。**实测不成立。**
 * 同样 512 MB / 64KB 块 / 释放一半的对照实验（Normal zone）：
 *
 *     基线        order-0=406   order-1=234   order-3=8119  order-4=6350
 *     顺序释放后  order-0=5000  order-1=5883  order-3=3254  order-4=2692
 *     随机释放后  order-0=6360  order-1=6598  order-3=2516  order-4=2502
 *
 * 随机确实更狠（低阶碎屑更多、中阶残留更少），但**顺序释放也造出了大量碎片**，
 * 远不是"白干"。
 *
 * 原因是原注释混淆了两个概念：**虚拟连续 ≠ 物理连续**。
 * "顺序"释放的是**虚拟地址**上连着的那一半，但这片虚拟内存背后的物理页，
 * 是缺页时内核从伙伴系统东一块西一块摘来的 —— 虚拟地址挨着，物理页并不挨着。
 * 而 __free_one_page() 的合并只看**物理页号（PFN）的邻居**，
 * 所以"顺序还"实际上还的是物理上散落各处的页，合并自然起不来。
 *
 * 那随机为什么还是更狠？因为内核给同一进程连续缺页时，倾向于从同一个
 * pageblock 里摘相邻的页（migratetype 分组的副作用），
 * 所以虚拟连续的那一半在物理上仍有**部分**聚集；
 * 顺序释放能捞回一些合并机会，随机释放把这点残余规律也打散了。
 *
 * → 结论：随机释放保留（它确实更有效），但不要再宣称"顺序释放等于没做"。
 *
